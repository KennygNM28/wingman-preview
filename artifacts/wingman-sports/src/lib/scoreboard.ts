...
export async function fetchScoreboard(league:LeagueKey,date:string):Promise<ScoreboardPayload>{
  ...
  try{const r=await fetch(`/api/sports/scoreboard?league=${league}&date=${date}`,{cache:'no-store'});return remember(await json(r) as ScoreboardPayload)}catch(e){failures.push(`Wingman API: ${e instanceof Error?e.message:'unavailable'}`)}

  if(league==='mlb'){
    try{return remember(await tryMlb(date))}catch(e){failures.push(`MLB: ${e instanceof Error?e.message:'unavailable'}`)}
  }
  if(league==='nba'){
    try{return remember(await tryNba(date))}catch(e){failures.push(`NBA.com: ${e instanceof Error?e.message:'unavailable'}`)}
  }

  try{return remember(await tryEspn(league,date))}catch(e){failures.push(`ESPN: ${e instanceof Error?e.message:'unavailable'}`)}
