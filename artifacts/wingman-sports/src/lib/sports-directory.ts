...
function rowsToTeams(league:LeagueKey, rows:Array<[string,string]>):DirectoryTeam[]{return rows.map(([abbreviation,name],i)=>({id:`local-${league}-${i}`,abbreviation,name,shortName:name,logo:null}))}
function parseEspnTeams(payload:any):DirectoryTeam[]{const rows=payload?.sports?.[0]?.leagues?.[0]?.teams;if(!Array.isArray(rows))return[];return rows.map((x:any)=>x?.team??x).filter((x:any)=>x?.id).map((x:any)=>({id:String(x.id),abbreviation:String(x.abbreviation??x.shortDisplayName??x.name??''),name:String(x.displayName??x.name??''),shortName:String(x.shortDisplayName??x.name??''),logo:typeof x?.logos?.[0]?.href==='string'?x.logos[0].href:null})).filter((x:DirectoryTeam)=>x.abbreviation&&x.name).sort((a:DirectoryTeam,b:DirectoryTeam)=>a.name.localeCompare(b.name))}
function parseMlbTeams(payload:any):DirectoryTeam[]{const rows=Array.isArray(payload?.teams)?payload.teams:[];return rows.map((x:any)=>({id:String(x?.id??''),abbreviation:String(x?.abbreviation??x?.teamCode??x?.fileCode??x?.shortName??''),name:String(x?.name??''),shortName:String(x?.shortName??x?.teamName??x?.name??''),logo:x?.id?`https://www.mlbstatic.com/team-logos/${x.id}.svg`:null})).filter((x:DirectoryTeam)=>x.id&&x.abbreviation&&x.name).sort((a,b)=>a.name.localeCompare(b.name))}

function readCache(league:LeagueKey):DirectoryTeam[]{if(typeof window==='undefined')return[];try{const raw=window.localStorage.getItem(`wingman:teams:${league}`),parsed=raw?JSON.parse(raw):[];return Array.isArray(parsed)?parsed.filter((x:any)=>x?.abbreviation&&x?.name):[]}catch{return[]}}
function writeCache(league:LeagueKey,teams:DirectoryTeam[]){if(typeof window==='undefined'||!teams.length)return;try{window.localStorage.setItem(`wingman:teams:${league}`,JSON.stringify(teams))}catch{/* storage unavailable */}}
async function jsonOrThrow(response:Response):Promise<any>{const text=await response.text();if(!text.trim())throw new Error(`Empty response (${response.status||'network'})`);let payload:any;try{payload=JSON.parse(text)}catch{throw new Error(`Invalid JSON response (${response.status||'network'})`)}if(!response.ok)throw new Error(payload?.detail||payload?.error||`Request failed (${response.status})`);return payload}

export function getTeamFallback(league:LeagueKey):DirectoryTeam[]{const cached=readCache(league);if(cached.length)return cached;const pro=PRO_TEAMS[league];if(pro?.length)return rowsToTeams(league,pro);const college=COLLEGE_PREVIEW_SEEDS[league];return college?.length?rowsToTeams(league,college):[]}

export async function fetchTeams(league:LeagueKey):Promise<DirectoryTeam[]>{
  try{const r=await fetch(`/api/sports/teams?league=${league}`,{cache:'no-store'}),p=await jsonOrThrow(r),teams=(p?.teams||[]) as DirectoryTeam[];if(teams.length){writeCache(league,teams);return teams}}catch{/* continue through provider chain */}

  if(league==='mlb'){
    try{const r=await fetch('https://statsapi.mlb.com/api/v1/teams?sportId=1',{cache:'no-store',headers:{Accept:'application/json'}}),teams=parseMlbTeams(await jsonOrThrow(r));if(teams.length){writeCache(league,teams);return teams}}catch{/* continue */}
  }

  try{const cfg=ESPN[league],url=`https://site.api.espn.com/apis/site/v2/sports/${cfg.sport}/${cfg.slug}/teams?limit=1000`,r=await fetch(url,{cache:'no-store',headers:{Accept:'application/json'}}),teams=parseEspnTeams(await jsonOrThrow(r));if(teams.length){writeCache(league,teams);return teams}}catch{/* use local/cached fallback */}

  const fallback=getTeamFallback(league);if(fallback.length)return fallback;throw new Error('Team directory unavailable')
}
